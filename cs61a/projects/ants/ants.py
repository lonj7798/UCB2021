...
# A single tile that an Ant can be placed on and that connects to other Places
class Place:
...
# The ThrowerAnt finds the nearest place including and in front of itsown place that has Bees and throws at a random Bee in that place
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        # self.place.bees: list
        # cases: no bees (including bees stay inside of the Hive) -> list is empty until to reach the hive
        #        bees -> list is not empty
        
        searching_place = self.place
        distance = 0 # prob4
        
        def searching(self, distance, searching_place):
            if searching_place.is_hive:
                return None
            
            if searching_place.bees and (self.min_range <= distance <= self.max_range):
                return random_bee(searching_place.bees)
            return searching(self, distance+1, searching_place.entrance)
        
        return searching(self, 0, self.place)
        
        
        """ # while loop will stop after found the first bees
        while (not searching_place.bees) and (not searching_place.is_hive):
        ...
        # prob4 
        if distance <= self.max_throw_range and distance >= self.min_throw_range:
            print(distance)
            return random_bee(searching_place.bees)
        """
...
class ShortThrower(ThrowerAnt):
    ...
    # initial health is 1 and default health of Ant object is 1
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        """
        # BEGIN Problem 5
        # super().reduce_health(amount) -> object data is disapeared
        
        # if self.health <= 0:  
        
        if self.health <= amount:
            copy_bees = self.place.bees[:] # need a copy of list bees / need to update the list inside of for-loop
            for bee in copy_bees:
                # reduce_health from insect class
