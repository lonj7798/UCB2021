...
def readSensorData(ser):
    ...
    if (serial_data[0] == -1 or serial_data[1] == -1):
        ...
        loc = (serial_data[0], serial_data[1])
    return "(" + str(loc[0]) + "," + str(loc[1]) + ")"

def get_ser_port():
    print("\nEECS16A Touchscreen 2 Lab\n")
...
def handshake(y):
    ser = get_ser_port()
    print("\nPING Launchpad with data request.\n")
    i = 0
    for _ in range (0,y):
        x = str(readSensorData(ser))
        print("Received " + x + " from Launchpad")
        if (x[0] == "("):
            i += 1;
        time.sleep(1)
    ser.close()
...
def run_gui(root, buttons, locs, ser, on, off):    
    while True:
        try:
            time.sleep(0.05)
            color = [off, off, off, off, off, off, off, off, off, off]
            index = locs[readSensorData(ser)]
            color[index] = on
            buttons[0].configure(bg=color[0])
            buttons[1].configure(bg=color[1])
            buttons[2].configure(bg=color[2])
            buttons[3].configure(bg=color[3])
            buttons[4].configure(bg=color[4])
            buttons[5].configure(bg=color[5])
            buttons[6].configure(bg=color[6])
            buttons[7].configure(bg=color[7])
            buttons[8].configure(bg=color[8])
            root.update()
        except Exception:
            ser.close()
            sys.exit()
...
def launch_gui():
    ...
    def quit_func(args=None):
        return quit_key(ser, root)

    root.bind('<Key>', quit_func)
    root.protocol("WM_DELETE_WINDOW", quit_func)
   
    t2 = threading.Thread(target = run_gui, args=(root, buttons, locs, ser, on, off), daemon=True)
    t2.start()
