...
def plot_speakers_demo():
    ...
    # Form the system equations and solve for the beacon positions and t0 delay.
    
    speakers = labDemo.beaconsLocation
    
    A = np.zeros([3,3])
    A[:,:2] = np.array(speakers[1:4])
    A[:,2] = v*v*TDOA[1:4]
    A = A*2;
    b = np.sum(speakers[1:4]*speakers[1:4],axis=1) - v*v*TDOA[1:4]*TDOA[1:4]
    ...
    y1 = (-A[0,0]*x - A[0,2]*r[2] + b[0])/A[0,1]
    y2 = (-A[1,0]*x - A[1,2]*r[2] + b[1])/A[1,1]
    y3 = (-A[2,0]*x - A[2,2]*r[2] + b[2])/A[2,1]
...
def construct_system_test(construct_system):
    ...
    A_ref, b_ref = labDemo.construct_system(speakers, TDOA, labDemo.V_AIR)
    result = labDemo.calculate_position(labDemo.least_squares, labDemo.construct_system, speakers, TDOA)
    
    for i in range(len(b)):
        
        calc_value = A[i][0] * coordinate[0] + A[i][1] * coordinate[1] + A[i][2] * result[2]
...
def user_test(Lab, construct_system, least_squares, isac=1):
    filename = input("Type filename (including the .wav): ")
    Lab.simulation_testing(filename, construct_system, least_squares)
